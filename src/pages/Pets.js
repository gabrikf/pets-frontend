...
const Pets = () => {
  ...
  const dislike = async(pet) => {
    ...
    await api.put(`/pets/${pet.id_pet}`, pet.likes, { headers: {
  ...
  return (
    <div>
          ...
          <Link onClick={() => setFilter('Cachorro')} to="/pets" ><img className="object-cover w-44 h-28" src={dogImg} alt="avatar"/></Link>
                
                <div className="border-t border-indigo-600 py-2 text-center dark:border-gray-400">
                    <Link onClick={() => setFilter('Cachorro')} to="/pets" className="block text-lg text-gray-800 dark:text-white">Cachorros</Link>
          ...
          <Link  onClick={() => setFilter('Perdido')} to="/pets"><img className="object-cover w-44  h-28" src={lostPet} alt="avatar"/></Link>
                
                <div className="mt- border-t border-indigo-600 py-2 text-center dark:border-gray-400">
                    <Link  onClick={() => setFilter('Perdido')} to="/pets" className="block text-base  text-gray-800 dark:text-white">Perdidos</Link>
          ...
          <div className="max-w-xs bg-blue-100 mx-auto border border-indigo-600 overflow-hidden bg-white rounded-full shadow-lg dark:bg-gray-800 dark:border-gray-400 ">
            <Link onClick={() => setFilter('Gato')} to="/pets" ><img className="object-cover  w-44  h-28" src={catImg} alt="avatar"/></Link>
              
              <div className="border-t border-indigo-600 py-2 text-center dark:border-gray-400">
              <Link onClick={() => setFilter('Gato')} to="/pets" className="block text-lg text-gray-800 dark:text-white">Gatos</Link>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-start  lg:mx-20 mx-5 bg-white  dark:bg-gray-800">
                { 
                
                  incidents.filter(incident => (incident.animal_type === kindOfResults || !kindOfResults)).map(incident => ( 
                ...
                <div key={incident.id_pet} className="m-4 max-w-sm  overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
                  { incident.images && 
                    ...
                    {login && <div>
                      {incident.likes[login.userId] ? <AiFillHeart onClick={() => dislike(incident)} className='my-2 text-3xl text-red-600 cursor-pointer'/> : <AiOutlineHeart onClick={() => like(incident)}className='cursor-pointer my-2 text-3xl text-red-300 '/>}
                        <p className="text-sm font-semibold text-gray-800 dark:text-white">  {Object.keys(incident.likes).length} curtida{Object.keys(incident.likes).length > 1 && "s"}</p>
                        </div>}
                          <h1 className="text-xl font-semibold text-gray-800 dark:text-white">{incident.name}</h1> 
                          <div className="py-2 text-gray-700 dark:text-gray-400"><strong className='text-black dark:text-white'>Idade do pet: </strong>{incident.pet_age}</div> 
                          <div className="h-24 py-2 text-gray-700 dark:text-gray-400"><strong className='text-black dark:text-white'>Descrição do pet: </strong> <p>{incident.description}</p></div>
                ...
                {page !== 0 && 
                  <button  className="md:m-2 w-full px-3 py-2 mt-6 text-xs font-medium text-white uppercase transition-colors duration-200 transform bg-indigo-600 rounded-md lg:w-auto hover:bg-indigo-500 focus:outline-none focus:bg-indigo-500"
